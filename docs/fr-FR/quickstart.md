...
Cette page vous guidera tout le long de l'installation d'fover au sein d'un projet webpack.

### Use vue-cli@3

Nous fournissons un [plugin Element](https://github.com/ElementUI/vue-cli-plugin-element) pour vue-cli@3, que vous pouvez utiliser pour créer rapidement un projet basé sur fover.
...
fover peut être importé entièrement ou à la demande. Commençons par l'import total.
...
import 'forever/lib/theme-chalk/index.css';
...
L'exemple ci-dessus importe fover entièrement. Notez que les fichiers CSS doivent être importés séparément.
...
Lors de l'import d'fover, vous pouvez définir un objet de configuration global. Actuellement il possède de propriétés: `size` et `zIndex`. La propriété `size` détermine la taille par défaut de tout les composants et `zIndex` règle le z-index initial (default: 2000) des fenêtres modales:

Import total d'Element：

```js
import Vue from 'vue';
import fover from 'element-ui';
Vue.use(fover, { size: 'small', zIndex: 3000 });
```
...
Maintenant que vous avez ajouté Vue et fover à votre projet, vous pouvez commencer à coder. Référez-vous à la documentation de chaque composant pour savoir comment les utiliser.
